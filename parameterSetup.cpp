...
void separateNameAndValue(std::string& aLine, std::string& aParamName, std::string& aParamValue) //分离值和名称
{
...
std::string removeEmptySpacesFromString(string input)//移除空格
{
    string output = input;
...
void parseDimensions2D(std::string& aParamValue, int& aX, int& aY, const char separator)//的主要功能是从一个包含二维尺寸（以特定分隔符分隔的两个整数）的字符串中解析出X和Y坐标值
{
...
void parseFloats(std::string& aParamValue, float& aX, float& aY, const char separator)// parseFloats 的函数，其主要功能是从一个包含两个浮点数（以特定分隔符分隔）的字符串中解析出X和Y坐标值，并将它们分别存储在传入的引用参数 aX 和 aY 中。以下是详细解释：
{
    for (unsigned int i = 0; i < aParamValue.length(); i++)
    {
        if (aParamValue[i] == separator)
...
void ParameterSetup::parseComFile(const string tiltcom)
{
    ...
    while (getline(ifs, line))
    {
        if (line[0] != '#' && line[0] != '$')
        {
            ...
            storeValues(paramName, paramValue, ' ');//存储命令
        }
    }
}
